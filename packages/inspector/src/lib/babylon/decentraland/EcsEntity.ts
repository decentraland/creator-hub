...
export class EcsEntity extends BABYLON.TransformNode {
  ...
  constructor(
    ...
    public context: WeakRef<SceneContext>,
  ...
  *childrenEntities(): Iterable<EcsEntity> {
  ...
  dispose(
    _doNotRecurse?: boolean | undefined,
    _disposeMaterialAndTextures?: boolean | undefined,
  ): void {
  ...
  getGroupMeshesBoundingBox() {
    ...
    const children = this.gltfContainer
      ? this.gltfContainer.getChildMeshes(false)
      : this.getChildMeshes(false);
  ...
  generateBoundingBox() {
    ...
    boundingInfoMesh.setBoundingInfo(
      new BABYLON.BoundingInfo(boundingVectors.minimum, boundingVectors.maximum),
    );
...
function removeOutsideLayoutMaterial(mesh: BABYLON.AbstractMesh) {
  if (
    ...
    mesh.material.name === 'entity_outside_layout_multimaterial'
  ) {
