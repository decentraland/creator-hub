...
import { Vector3, TransformNode, Quaternion, Color3 } from '@babylonjs/core';
...
export class ScaleGizmo implements IGizmoTransformer {
  ...
  private planeMaterials: Map<any, { material: any, originalDiffuse: any, originalEmissive: any }> = new Map(); // Store material refs
  ...
  private createPlaneCubes(): void {
    ...
    const xyPlane = BABYLON.MeshBuilder.CreateBox('scalePlaneXY', {
      width: planeSize,
      height: planeSize,
      depth: thickness,
    }, scene);
    xyPlane.position.set(offset, offset, 0);
    const xyMaterial = new BABYLON.StandardMaterial('xyPlaneMat', scene);
    xyMaterial.diffuseColor = new BABYLON.Color3(0.5, 0.7, 1.0); // Light blue (Blender style)
    xyMaterial.emissiveColor = new BABYLON.Color3(0.3, 0.4, 0.6); // Slight glow
    xyMaterial.alpha = 0.7;
    xyMaterial.disableLighting = true;
    xyPlane.material = xyMaterial;
    xyPlane.parent = rootMesh;
    xyPlane.isPickable = true;
    
    // XZ Plane (light green) - thin on Y axis, scales X and Z
    const xzPlane = BABYLON.MeshBuilder.CreateBox('scalePlaneXZ', {
      width: planeSize,
      height: thickness,
      depth: planeSize,
    }, scene);
    xzPlane.position.set(offset, 0, offset);
    const xzMaterial = new BABYLON.StandardMaterial('xzPlaneMat', scene);
    xzMaterial.diffuseColor = new BABYLON.Color3(0.5, 1.0, 0.5); // Light green (Blender style)
    xzMaterial.emissiveColor = new BABYLON.Color3(0.3, 0.6, 0.3); // Slight glow
    xzMaterial.alpha = 0.7;
    xzMaterial.disableLighting = true;
    xzPlane.material = xzMaterial;
    xzPlane.parent = rootMesh;
    xzPlane.isPickable = true;
    
    // YZ Plane (light red) - thin on X axis, scales Y and Z
    const yzPlane = BABYLON.MeshBuilder.CreateBox('scalePlaneYZ', {
      width: thickness,
      height: planeSize,
      depth: planeSize,
    }, scene);
    yzPlane.position.set(0, offset, offset);
    const yzMaterial = new BABYLON.StandardMaterial('yzPlaneMat', scene);
    yzMaterial.diffuseColor = new BABYLON.Color3(1.0, 0.5, 0.5); // Light red (Blender style)
    yzMaterial.emissiveColor = new BABYLON.Color3(0.6, 0.3, 0.3); // Slight glow
    yzMaterial.alpha = 0.7;
    yzMaterial.disableLighting = true;
    yzPlane.material = yzMaterial;
    yzPlane.parent = rootMesh;
    yzPlane.isPickable = true;
    ...
    this.planeMaterials.set(xyPlane, { 
      material: xyMaterial, 
      originalDiffuse: new BABYLON.Color3(0.5, 0.7, 1.0), 
      originalEmissive: new BABYLON.Color3(0.3, 0.4, 0.6), 
    });
    this.planeMaterials.set(xzPlane, { 
      material: xzMaterial, 
      originalDiffuse: new BABYLON.Color3(0.5, 1.0, 0.5), 
      originalEmissive: new BABYLON.Color3(0.3, 0.6, 0.3), 
    });
    this.planeMaterials.set(yzPlane, { 
      material: yzMaterial, 
      originalDiffuse: new BABYLON.Color3(1.0, 0.5, 0.5), 
      originalEmissive: new BABYLON.Color3(0.6, 0.3, 0.3), 
    });
  ...
  private addPlaneHoverBehavior(planeMesh: any): void {
    ...
    const hoverDiffuse = new BABYLON.Color3(1.0, 1.0, 0.0); // Bright yellow
    const hoverEmissive = new BABYLON.Color3(0.8, 0.8, 0.0); // Yellow glow
    ...
    const onPointerExit = (pointerInfo: any) => {
      ...
      if ((!pickInfo || !pickInfo.hit || pickInfo.pickedMesh !== planeMesh) && this.activelyDraggingPlane !== planeMesh) {
  ...
  private addPlaneDragBehavior(planeMesh: any, planeType: 'XY' | 'XZ' | 'YZ'): void {
    ...
    const onPointerDown = (pointerInfo: any) => {
      ...
      const matInfo = this.planeMaterials.get(planeMesh);
      if (matInfo) {
        matInfo.material.diffuseColor = new BABYLON.Color3(1.0, 1.0, 0.0); // Yellow
        matInfo.material.emissiveColor = new BABYLON.Color3(0.8, 0.8, 0.0); // Yellow glow
      }
    ...
    const onPointerMove = (pointerInfo: any) => {
      if (!isDragging || !initialMousePos || pointerInfo.type !== BABYLON.PointerEventTypes.POINTERMOVE) return;
    ...
    const onPointerUp = (pointerInfo: any) => {
      if (!isDragging || pointerInfo.type !== BABYLON.PointerEventTypes.POINTERUP) return;
  ...
  private configureUniformScaleGizmo(): void {
    ...
    if (existingMesh && !uniformGizmo._customMeshSet) {
      ...
      // Import MeshBuilder and materials
      const BABYLON = require('@babylonjs/core');
      
      // Create a proper cube to replace the hexagonal prism (15% smaller)
      const cubeSize = 0.0102; // 0.012 * 0.85 = 15% smaller
      const customCube = BABYLON.MeshBuilder.CreateBox('uniformScaleCube', {
        size: cubeSize,
      }, scene);
      
      // Create a white/yellow material like the original gizmo
      const material = new BABYLON.StandardMaterial('uniformScaleCubeMat', scene);
      material.diffuseColor = new BABYLON.Color3(1, 1, 1); // White
      material.emissiveColor = new BABYLON.Color3(0.8, 0.8, 0.5); // Slight yellow glow
