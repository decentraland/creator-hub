...
export const getLayoutManager = memoize((scene: Scene) => {
    ...
    for (const parcel of parcels) {
      ...
      plane.setBoundingInfo(
        new BoundingInfo(
          plane
            .getBoundingInfo()
            .boundingBox.minimum.add(new Vector3(0, 0, -Math.log2(parcels.length + 1) * 20)),
  ...
  function isEntityOutsideLayout(mesh: AbstractMesh) {
    ...
    const boundingBoxCorners = [
      ...
      new Vector3(
        meshBoundingBox.minimumWorld.x,
        meshBoundingBox.minimumWorld.y,
        meshBoundingBox.maximumWorld.z,
      ),
      new Vector3(
        meshBoundingBox.minimumWorld.x,
        meshBoundingBox.maximumWorld.y,
        meshBoundingBox.minimumWorld.z,
      ),
      new Vector3(
        meshBoundingBox.minimumWorld.x,
        meshBoundingBox.maximumWorld.y,
      ...
      new Vector3(
        meshBoundingBox.maximumWorld.x,
        meshBoundingBox.minimumWorld.y,
        meshBoundingBox.minimumWorld.z,
      ),
      new Vector3(
