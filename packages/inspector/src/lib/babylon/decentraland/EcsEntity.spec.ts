...
describe('EcsEntity', () => {
  ...
  it('should add component with putComponent correctly', () => {
    ...
    expect(mockedContext.componentPutOperations[Transform.componentId]).toBeCalledWith(
      entity,
      Transform,
    );
  });
  ...
  it('should remove component with deleteComponent correctly', () => {
    ...
    expect(entity.usedComponents.size).toBe(0);
    expect(mockedContext.componentDeleteOperations[Transform.componentId]).toBeCalledWith(
      entity,
      Transform,
    );
  });
  ...
  it('components and native engine', () => {
    ...
    const deleteComponentMock = vi.spyOn(entity, 'deleteComponent');
