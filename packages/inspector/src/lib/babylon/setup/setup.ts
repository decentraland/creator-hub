...
function reposition(
  ...
  sunPositionColor.r = sunPosition.x = 500 * Math.cos(phi);
  sunPositionColor.g = sunPosition.y = 500 * Math.sin(phi) * Math.sin(theta);
  sunPositionColor.b = sunPosition.z = 500 * Math.sin(phi) * Math.cos(theta);

  const sunfade = 1.0 - Math.min(Math.max(1.0 - Math.exp(sunPosition.y / 10), 0.0), 0.9);
