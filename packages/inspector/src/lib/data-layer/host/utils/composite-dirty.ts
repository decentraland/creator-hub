...
} from './engine-to-composite';
import type { CompositeManager } from './fs-composite-provider';
...
export async function compositeAndDirty(
  ...
  async function dumpEngineAndGetComposite(
    dump: boolean = true,
  ): Promise<CompositeDefinition | null> {
    try {
