...
/**
 * Generates the content of the TypeScript spawn helper file for a Custom Item.
 *
 * The generated file exports a `spawn(options?)` function that scene TypeScript
 * code can import and call to dynamically create an instance of the custom item
 * at runtime using `@dcl/asset-packs`'s `spawnComposite`.
 *
 * @param itemName - Human-readable name of the custom item (used in JSDoc).
 * @param composite - The composite definition to embed.
 * @param assetPath - Scene-relative path to the custom item folder (e.g. `custom/monster`).
 */
function generateSpawnHelper(itemName: string, composite: unknown, assetPath: string): string {
  const compositeJson = JSON.stringify(composite, null, 2);

  return `// This file is auto-generated by Creator Hub. Do not edit manually.
// To update it, re-save the Custom Item in the Inspector.
import type { Entity } from '@dcl/ecs'
import { engine } from '@dcl/sdk/ecs'
import { spawnComposite } from '@dcl/asset-packs'
import type { SpawnCompositeOptions, AssetComposite } from '@dcl/asset-packs'

const composite: AssetComposite = ${compositeJson}

/**
 * Spawns a new instance of the "${itemName}" Custom Item.
 *
 ...
 * @example
 * \`\`\`typescript
 * import { spawn } from './${assetPath}'
 * import { Vector3 } from '@dcl/sdk/math'
 *
 * const monster = spawn({ position: Vector3.create(5, 0, 5) })
 * \`\`\`
 */
export function spawn(options?: SpawnCompositeOptions): Entity {
  return spawnComposite(engine, composite, '${assetPath}', options)
}
`;
}
  ...
  return {
    ...
    async createCustomAsset(req) {
      return stateManager.executeTransaction('external', async () => {
        ...
        const parsedComposite = JSON.parse(new TextDecoder().decode(composite));

        await fs.writeFile(
          `${customAssetPath}/composite.json`,
          Buffer.from(JSON.stringify(parsedComposite, null, 2)),
        );

        await fs.writeFile(
          `${customAssetPath}/index.ts`,
          Buffer.from(generateSpawnHelper(name, parsedComposite, customAssetPath)),
        );
            ...
            try {
              ...
              for (const file of files) {
                if (file.endsWith('data.json')) {
                  ...
                  // index.ts is a generated TypeScript spawn helper — not an asset resource
                } else {
                  resources.push(file);
                }
              }
