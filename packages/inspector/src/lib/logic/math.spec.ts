...
describe('getWorldMatrix', () => {
  ...
  it('should correctly compute world matrixes of entities at the first three levels of hierarchy', () => {
    ...
    expect(areSRTMatrixesEqualWithEpsilon(getWorldMatrix(e1, transform), Matrix.Identity())).toBe(
      true,
    );
    expect(
      areSRTMatrixesEqualWithEpsilon(
        getWorldMatrix(e2, transform),
        Matrix.compose({ x: 2, y: 2, z: 2 }, Quaternion.fromEulerDegrees(45, 45, 45), {
          x: 1,
          y: 1,
          z: 1,
        }),
    ...
    expect(
      areSRTMatrixesEqualWithEpsilon(
        ...
        Matrix.compose(
          { x: 4, y: 4, z: 4 },
          Quaternion.fromEulerDegrees(34.32064917, 98.0173426, 102.24704707),
          {
            x: 5.82842712,
            y: 2.17157287,
            z: 5.82842712,
