import type { Entity, TransformComponentExtended } from '@dcl/ecs';
...
export function areSRTMatrixesEqualWithEpsilon(
  m1: Matrix.ReadonlyMatrix,
  m2: Matrix.ReadonlyMatrix,
) {
  const tOut1 = Vector3.create();
  const sOut1 = Vector3.create();
  const rOut1 = Quaternion.create();

  if (!Matrix.decompose(m1, sOut1, rOut1, tOut1))
    throw new Error(`${Matrix.toArray(m1)} is not an SRT matrix`);

  const tOut2 = Vector3.create();
  const sOut2 = Vector3.create();
  const rOut2 = Quaternion.create();

  if (!Matrix.decompose(m2, sOut2, rOut2, tOut2))
    throw new Error(`${Matrix.toArray(m2)} is not an SRT matrix`);
