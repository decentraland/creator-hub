...
import type { LoadableScene } from '../../babylon/decentraland/SceneContext';
    ...
    entity: {
      ...
      content: catalog
        .map(assetPack =>
          assetPack.assets
            .map(asset =>
              Object.keys(asset.contents).map(file => ({ file, hash: asset.contents[file] })),
            )
            .reduce(
              (assetPackContents, assetContents) => assetPackContents.concat(assetContents),
              [],
            ),
        )
