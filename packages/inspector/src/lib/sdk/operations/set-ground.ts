...
import { Transform as TransformEngine, GltfContainer as GltfContainerEngine } from '@dcl/ecs';
...
export function setGround(engine: IEngine) {
  return function setGround(src: string) {
    ...
    const GltfContainer = engine.getComponent(
      GltfContainerEngine.componentName,
    ) as typeof GltfContainerEngine;
