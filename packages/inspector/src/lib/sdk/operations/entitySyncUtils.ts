 ...
 */
export function getEntitiesSyncStatus(
  sdk: SdkContextValue,
  entities: Entity[],
): Record<Entity, EntitySyncResult> {
  ...
  entities.forEach(entity => {
    ...
    const syncComponentsValue = hasSyncComponents
      ? sdk.components.SyncComponents.get(entity)
      : null;
