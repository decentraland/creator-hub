import type { IEngine, TransformComponentExtended, Entity, Name } from '@dcl/ecs';
import { Engine, engine } from '@dcl/ecs';
import * as components from '@dcl/ecs/dist/components';
import type { EditorComponents } from '../components';
import { EditorComponentNames, createEditorComponents } from '../components';
...
describe('duplicateEntity', () => {
  ...
  beforeEach(() => {
    ...
    SelectionComponent = engine.getComponent(
  ...
  it('should correctly duplicate an entity', () => {
    ...
    expect(NameComponent.get(duplicateChild!).value).toBe(
      `${NameComponent.get(originalChild).value}_2`,
    );
