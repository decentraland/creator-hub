...
export class ThreeRenderer implements IRenderer {
  ...
  constructor(
    ...
    // consumer subscribing synchronously after `new ThreeRenderer()` still gets
    // it). `cameraChange` is intentionally not emitted: this minimal renderer has
    // no user-driven camera controls, so there is no camera motion to report.
    queueMicrotask(() => {
  ...
  async getEntityAnimations(entity: Entity): Promise<RendererAnimation[]> {
    // Names only: a three.js AnimationClip carries no authored playback defaults
    // (loop/speed/weight are playback-time state, not clip data), so the optional
    // RendererAnimation fields are omitted and the inspector applies its defaults.
