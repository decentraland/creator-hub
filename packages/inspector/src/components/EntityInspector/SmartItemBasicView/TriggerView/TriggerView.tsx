...
export default React.memo(
  withSdk<WithSdkProps & { entity: Entity; field: ConfigComponent['fields'][0] }>(
    ({ sdk, entity, field }) => {
      ...
      const [triggerComponent, setTriggerComponentValue, isTriggerComponentEqual] =
        useComponentValue<EditorComponentsTypes['Triggers']>(entity, Triggers);
      const [triggers, , modifyTrigger] = useArrayState<Trigger>(
        triggerComponent === null ? [] : triggerComponent.value,
      );
      const [_, getActionEntity, getActionValue] = useComponentsWith(
      ...
      const areValidActions = useCallback(
        (updatedActions: TriggerAction[]) =>
          updatedActions.every(action => action.id && action.name),
        [],
      );
      ...
      const handleAddAction = useCallback(
        (trigger: Trigger) => {
          const triggerIdx = triggers.findIndex(
            _trigger => _trigger.basicViewId === trigger.basicViewId,
          );
          if (triggerIdx !== -1) {
          ...
          >
            <div className="TriggerActionContainer">
              {trigger.actions.map((action, idx) => {
                ...
                const isBasicViewEnabled =
                  Config.getOrNull(actionEntity as Entity)?.isBasicViewEnabled === true;
                if (
                  isBasicViewEnabled &&
                  actionId !== undefined &&
                  action.name !== '' &&
                  !availableActions
                    .get(actionId)
                    ?.actions.find(_action => _action.name === action.name)
                )
                  return null;
                  ...
                  >
                    <div className="Fields">
                      ...
                      <Dropdown
                        placeholder="Action"
                        disabled={
                          !action.id || !trigger.actions.find(_action => _action.id === action.id)
                        }
                        options={actions}
                        value={action.name}
                        searchable
                    ...
                    <div className="RightMenu">
                      <MoreOptionsMenu>
                        <RemoveButton onClick={e => handleRemoveAction(e, idx, trigger)}>
                          Remove Action
                        </RemoveButton>
                      </MoreOptionsMenu>
                    </div>
