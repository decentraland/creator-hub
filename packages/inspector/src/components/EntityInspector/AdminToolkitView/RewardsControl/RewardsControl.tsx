...
const RewardsControl: React.FC<WithSdkProps & Props> = ({ sdk, ...props }) => {
  ...
  const handleAddRewardItem = useCallback(() => {
    setAdminComponent({
      ...
      rewardsControl: {
        ...
        rewardItems: [
          ...(adminComponent.rewardsControl.rewardItems || []),
          { entity: 0, customName: '' },
        ],
      },
    });
  ...
  const handleRemoveRewardItem = useCallback(
    (idx: number) => {
      const updatedRewardItems = adminComponent.rewardsControl.rewardItems?.filter(
        (_, index) => index !== idx,
      );
  ...
  return (
    <div className="RewardsControl">
      {adminComponent.rewardsControl.rewardItems?.map((rewardItem, idx) => {
        ...
        return (
          <Block
            key={rewardItem.entity}
            className="RewardItemRow"
          >
