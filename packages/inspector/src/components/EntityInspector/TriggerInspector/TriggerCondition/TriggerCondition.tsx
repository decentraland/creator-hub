...
import type { TriggerCondition } from '@dcl/asset-packs';
import { TriggerConditionOperation, TriggerConditionType } from '@dcl/asset-packs';
...
export const TriggerConditionContainer = ({
  ...
  const sdk = useSdk();
  const [conditions, addCondition, modifyCondition, removeCondition] =
    useArrayState<TriggerCondition>(trigger.conditions as TriggerCondition[]);
  ...
  const handleChangeInputValue = useCallback(
    (
      { target: { value } }: React.ChangeEvent<HTMLSelectElement | HTMLInputElement>,
      idx: number,
    ) => {
      modifyCondition(idx, {
        ...conditions[idx],
  ...
  return (
    <div className="TriggerConditionsContainer">
      ...
      {conditions.map((condition, idx) => {
        ...
        const stateOptions = (
          (condition.id && availableStates.get(condition.id)?.states) ||
          []
        ).map(state => ({
          value: state,
          label: state,
        ...
        const isCounterCondition = counterConditionTypeOptions.some(
        ...
        return (
          <div
            ...
            key={`trigger-condition-${idx}`}
          >
            <div className="Fields">
              <EntityField
                components={
                  [
                    sdk?.components.States,
                    sdk?.components.Counter,
