...
export const actionsConditionTypeOptions = [
  {
    value: TriggerConditionType.WHEN_DISTANCE_TO_PLAYER_LESS_THAN,
    text: 'distance to player is less than',
  },
  {
    value: TriggerConditionType.WHEN_DISTANCE_TO_PLAYER_GREATER_THAN,
    text: 'distance to player is greater than',
  },
];
  ...
  const availableActions: Map<number, { name: string; actions: Action[] }> = useMemo(() => {
    return entitiesWithAction?.reduce((actions, entityWithAction) => {
      ...
      if (actionsComponentValue.value.length > 0) {
        actions.set(actionsComponentValue.id, {
          name: name,
          actions: actionsComponentValue.value as Action[],
        });
      }
    ...
    return entitiesWithStates?.reduce((states, entityWithState) => {
      ...
      if (statesComponentValue.value.length > 0) {
        states.set(statesComponentValue.id, {
          name: name,
          states: (statesComponentValue as States).value,
        });
      }
          ...
          >
            <>
              ...
              <TriggerActionContainer
                ...
                onUpdateActions={(actions: TriggerAction[]) =>
                  handleUpdateActions(actions, triggerIdx)
                }
              />
            </>
          </TriggerEvent>
