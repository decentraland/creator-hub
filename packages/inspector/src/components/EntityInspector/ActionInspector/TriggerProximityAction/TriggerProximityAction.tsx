...
const TriggerProximityAction: React.FC<Props> = ({ value, onUpdate }: Props) => {
  ...
  const renderRadiusInfo = useMemo(
    () => (
      <InfoTooltip
  ...
  const renderLayerInfo = useMemo(
    () => (
      <InfoTooltip
        text="Select which player layers are affected by the damage. 'All' affects all players, 'Player' affects only players, 'Pointer' affects pointer interactions."
        position="top center"
      />
    ),
    [],
