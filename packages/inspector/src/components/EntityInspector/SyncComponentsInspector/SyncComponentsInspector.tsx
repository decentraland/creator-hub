...
export default withSdk<Props>(({ sdk, entity, initialOpen = true }) => {
  ...
  const [componentValue, setComponentValue] = useComponentValue<ISyncComponentsType>(
    entity,
    SyncComponents,
  );
  ...
  useChange(({ entity: eventEntity, component, operation }) => {
    ...
    if (eventEntity !== entity || !component || !ENABLED_COMPONENTS.has(component.componentName))
      return;
    ...
    if (operation === CrdtMessageType.DELETE_COMPONENT) {
      setComponentValue({
        componentIds: deleteComponentIds(sdk.engine, entity, componentIds, component),
      });
    }
  }, []);
  ...
  const handleChange = useCallback(
    (componentId: number) => {
      if (componentValue.componentIds.includes(componentId)) {
        setComponentValue({
          componentIds: componentValue.componentIds.filter($ => $ !== componentId),
        });
      } else {
