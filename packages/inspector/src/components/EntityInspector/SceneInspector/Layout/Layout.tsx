...
function Layout({ value, onChange }: Props) {
  ...
  const isTileDisabled = useCallback(
    (coord: Coords) => !hasCoord(enabledCoords, coord),
    [enabledCoords],
  );

  const isBaseTile = useCallback(
    (coord: Coords) => coord.x === base.x && coord.y === base.y,
    [base],
  );

  const handleTileClick = useCallback((coord: Coords) => {
    setGrid(prevGrid =>
      prevGrid.map($ => (isCoord($, coord) ? { ...$, disabled: !$.disabled } : $)),
    );
  }, []);
      ...
      {mode === Mode.ADVANCED ? (
        ...
        <>
          ...
          >
            ...
            <Button
              type="dark"
              onClick={handleModeChange(Mode.ADVANCED)}
            >
