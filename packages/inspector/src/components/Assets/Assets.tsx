...
import { useScanOptimizableAssets } from '../../hooks/useScanOptimizableAssets';
...
function Assets({ isAssetsPanelCollapsed }: { isAssetsPanelCollapsed: boolean }) {
  ...
  const [selectedOptimizeAssets, setSelectedOptimizeAssets] = useState<Set<string>>(new Set());
  ...
  const {
    ...
    isScanning: isOptimizeScanning,
    scanAssets: scanOptimizableAssets,
  } = useScanOptimizableAssets();
  ...
  const handleScanOptimizableAssets = useCallback(async () => {
    const assets = await scanOptimizableAssets();
    setSelectedOptimizeAssets(new Set(assets.map(a => a.path)));
  }, [scanOptimizableAssets]);

  const handleOptimizeClick = useCallback(async () => {
    setShowOptimizeModal(true);
    handleScanOptimizableAssets();
  }, [handleScanOptimizableAssets]);

  const handleCloseOptimizeModal = useCallback(() => {
    setSelectedOptimizeAssets(new Set());
    setShowOptimizeModal(false);
  }, []);

  const handleSelectOptimizeAsset = useCallback((value: string) => {
    setSelectedOptimizeAssets(prevSet => {
      const newSet = new Set(prevSet);
  ...
  const handleSelectAllOptimizeAssets = useCallback(
    (selectAll: boolean) => {
      if (selectAll) {
        setSelectedOptimizeAssets(new Set(optimizableAssets.map(a => a.path)));
      } else {
        setSelectedOptimizeAssets(new Set());
      }
    },
  ...
  return (
    <div className="Assets">
      ...
      <OptimizeAssets
        ...
        assets={optimizableAssets}
        isScanning={isOptimizeScanning}
        selectedAssets={selectedOptimizeAssets}
        onClose={handleCloseOptimizeModal}
        onScan={handleScanOptimizableAssets}
        onSelect={handleSelectOptimizeAsset}
        onSelectAll={handleSelectAllOptimizeAssets}
      />
    </div>
  );
}
