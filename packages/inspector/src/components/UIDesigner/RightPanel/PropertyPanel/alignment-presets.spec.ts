...
describe('alignment presets', () => {
  ...
  // The whole point of resolving against flexDirection: "Top left" has to mean top
  // left on screen in every direction. For a row the main axis is horizontal, so
  // `left` is justifyContent; for a column it is vertical, so `top` is.
  describe('and the flex direction decides which prop owns which screen axis', () => {
    ...
    it('should flip start and end on a reversed main axis', () => {
      // row-reverse runs right→left, so "left" is the END of the main axis.
      expect(alignmentToPatch('top-left', ROW_REVERSE)).toEqual({
        justifyContent: JUSTIFY_END,
        alignItems: ALIGN_START,
      });
      // column-reverse runs bottom→top, so "top" is the END of the main axis.
  ...
  // Anything outside the 9 has no cell; the panel shows "Default" rather than
  // pretending a distribution or a stretch is an alignment.
