...
// Queried by label and by role attribute rather than through getByRole: under
// happy-dom the accessibility-tree filter reports the whole panel inaccessible
// (the same reason FlowField.spec reaches for querySelector).
...
describe('when the menu is open', () => {
  ...
  // One handler driving both Mouse Down and Mouse Up is legitimate, and a
  // hand-authored reuse must not vanish from its own dropdown, so the list is
  // never filtered by what is already bound elsewhere.
...
describe('when adding an action from inside the menu', () => {
  ...
  it('should seed the name from the event and declare it, then bind it', async () => {
    ...
    // The bind is what the await in commitNew is FOR: it must land after the
    // declaration has been spliced and reparsed, never before.
    await vi.waitFor(() =>
      expect(store.bindAttribute).toHaveBeenCalledWith(
        7,
        'onMouseDown',
  ...
  // The name is spliced into source as code, so both guards are load-bearing.
