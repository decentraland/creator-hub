...
// The handler is bound on the group and reads its children as the segments, so a
// bare DOM group is all it needs. It has to be in the document for focus to land.
...
describe('the segmented control keyboard pattern', () => {
  // Selection follows focus in an ARIA radio group: the arrows do both.
  ...
  describe('when assigning the roving tabindex', () => {
    ...
    // Otherwise Tab could reach no segment at all, stranding the group.
