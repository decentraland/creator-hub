...
const getExpandStyles = (active: boolean) => ({
  ...
  return React.memo(
    withContextMenu<Props<T>>(props => {
      ...
      const controlsProps = {
        ...
        enableEdit:
          (enableRename &&
            (!isEntity || (sdk && sdk.operations.getSelectedEntities().length < 2))) ||
          false,
        enableRemove,
        enableDuplicate,
      ...
      return (
        ...
        >
          ...
          >
            ...
            <div
              ref={ref}
              style={getEditModeStyles(editMode)}
              className="item-area"
            >
              <DisclosureWidget
                enabled={enableOpen}
          ...
          {insertMode && (
            <Input
              value=""
              onCancel={quitInsertMode}
              onSubmit={handleAddChild}
              onBlur={quitInsertMode}
            />
          )}
        </div>
      );
