...
let useContextMock: vi.Mock;

const mockComponents = [
  { componentId: 1, componentName: 'Transform', has: vi.fn() },
  { componentId: 2, componentName: 'Mesh Renderer', has: vi.fn() },
  { componentId: 3, componentName: 'Visibility', has: vi.fn() },
];

const mockSdk = {
  engine: {
    componentsIter: vi.fn().mockReturnValue(mockComponents),
    getComponentOrNull: vi.fn((id: number) => mockComponents.find(comp => comp.componentId === id)),
  ...
  components: {
    Transform: { componentId: 1, componentName: 'Transform', has: vi.fn() },
    MeshRenderer: { componentId: 2, componentName: 'Mesh Renderer', has: vi.fn() },
    VisibilityComponent: { componentId: 3, componentName: 'Visibility', has: vi.fn() },
  },
};

vi.mock('./useChange', () => ({
  useChange: vi.fn().mockImplementation((callback: any) => {
...
describe('useEntityComponent', () => {
  ...
  beforeEach(() => {
    ...
    useContextMock = React.useContext = vi.fn().mockReturnValue({ sdk: mockSdk }) as any;
    vi.clearAllMocks();
