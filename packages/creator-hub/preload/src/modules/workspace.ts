...
import { getScene, getRowsAndCols, parseCoords, pathToPosix, updateSceneThumbnail } from './scene';
...
export function initializeWorkspace(services: Services) {
  ...
  /**
   * Folder under `assets/` where the autogenerated thumbnail lives. Capitalised to match the
   * folder the asset importer creates for images (`assets/Images/`), the SDK docs and the
   * SDK skills. Mixing casings is invisible on macOS and Windows but breaks deployment: the
   * uploaded file list keeps on-disk casing and the Catalyst matches `navmapThumbnail`
   * against it case-sensitively.
   */
  const THUMBNAIL_FOLDER = 'Images';
  const THUMBNAIL_FILENAME = 'autogenerated-thumbnail.png';

  function getProjectThumbnailPath(imagesFolder: string = THUMBNAIL_FOLDER) {
    return path.join('assets', imagesFolder, THUMBNAIL_FILENAME);
  }

  /**
   * Name of the project's images folder as it exists on disk. Projects created before the
   * folder was capitalised have `assets/images/`; on a case-insensitive filesystem writing
   * `assets/Images/...` lands in that folder anyway, so the path recorded in scene.json must
   * use the casing that is really there. Falls back to the canonical name when no folder
   * exists yet.
   */
  async function resolveImagesFolderName(projectPath: string): Promise<string> {
    let entries: string[] = [];
    try {
      entries = await fs.readdir(path.join(projectPath, 'assets'));
    } catch (_) {
      return THUMBNAIL_FOLDER;
    }
    if (entries.includes(THUMBNAIL_FOLDER)) return THUMBNAIL_FOLDER;
    const wanted = THUMBNAIL_FOLDER.toLowerCase();
    return entries.find(entry => entry.toLowerCase() === wanted) ?? THUMBNAIL_FOLDER;
  }
  ...
  async function getProjectThumbnailAsBase64(projectPath: string, scene?: Scene): Promise<string> {
    try {
      ...
      try {
        // if there is no thumbnail defined in scene.json, look for the autogenerated one inside assets/Images/
        const imagesFolder = await resolveImagesFolderName(projectPath);
        const thumbnailPath = path.join(projectPath, getProjectThumbnailPath(imagesFolder));
        return (await fs.readFile(thumbnailPath)).toString('base64');
      } catch (_) {
        try {
  ...
  async function saveThumbnail({
    ...
    const thumbnailPath = getProjectThumbnailPath(await resolveImagesFolderName(scenePath));
    await fs.writeFile(path.join(scenePath, thumbnailPath), thumbnailContent, {
      encoding: 'base64',
    });

    // Point scene.json at the new thumbnail when it has none, when the one it names is gone,
    // or when it names the autogenerated thumbnail with a casing that differs from the folder
    // on disk (a case-insensitive filesystem reports that file as existing, but a deployment
    // of it is rejected because the uploaded path does not match).
    const currentThumb = scene.display?.navmapThumbnail;
    const currentThumbPath = path.join(scenePath, path.normalize(currentThumb || ''));
    const expectedThumb = pathToPosix(thumbnailPath);
    const isMiscasedAutoThumbnail =
      !!currentThumb &&
      currentThumb !== expectedThumb &&
      currentThumb.toLowerCase() === expectedThumb.toLowerCase();
    if (!currentThumb || isMiscasedAutoThumbnail || !(await fs.exists(currentThumbPath))) {
