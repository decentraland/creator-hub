...
const SpinnerComponent = engine.defineComponent('clap-claim-spinner', {
  angle: Schemas.Number,
  speed: Schemas.Number,
});

export class ClapClaim {
   ...
   * @param emoteDetected - The emote that the player must perform in front of the booth to trigger the claim
  ...
  constructor(
    ...
    public wearableYOffset: number = 0.25,
    ...
    if (startDate && !this.startDateParsed) {
      console.log(`[ClapClaim] Could not parse startDate: "${startDate}"`);
    }
    if (endDate && !this.endDateParsed) {
      console.log(`[ClapClaim] Could not parse endDate: "${endDate}"`);
    }
  }
  ...
  start() {
    ...
    Transform.create(triggerEntity, {
      ...
      scale: Vector3.create(1.75, 3, 1.75),
      parent: this.entity,
    });
  ...
  private initSpinner() {
    ...
    // Rotate the spinner UVs only while showing WAITING UI
    engine.addSystem(
      (dt: number) => {
      ...
      undefined,
      `clap-claim-spinner-${this.entity}`,
    );
  }
  ...
  public displayWearable() {
    ...
    Tween.setRotateContinuous(this.wearableEntity, Quaternion.fromEulerDegrees(0, -90, 0), 1);
  }
  ...
  private initClapToClaim() {
    AvatarEmoteCommand.onChange(engine.PlayerEntity, emote => {
      if (!this.playerInTrigger) return;
      if (!emote) return;
      console.log('Emote played: ', emote.emoteUrn);

      const performedEmote = (emote.emoteUrn ?? '').toLowerCase();
      const detectedEmote = (this.emoteDetected ?? '').toLowerCase();

      // Accept exact match OR detected string as substring of the performed emote
      // e.g. detected "clap" matches "urn:decentraland:off-chain:base-emotes:clap"
      if (detectedEmote && performedEmote.includes(detectedEmote)) {
        setTimeout(() => {
          this.claim();
        }, 1500);
      }
