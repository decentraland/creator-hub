...
>>>>>>> 1ed397a (fix thumbnail on publish)
}

export function getProjectThumbnailPath() {
  return path.join('assets', 'hub', '__thumbnail__.png');
}
 ...
 * if the current thumbnail is: the one generated by the hub or missing or invalid.
 *
...
export async function saveThumbnail({
  ...
  const isUsingHubThumbnail =
    currentThumb === thumbnailPath ||
    !currentThumb ||
    !(await exists(path.join(_path, currentThumb)));

  if (isUsingHubThumbnail) await updateSceneThumbnail(_path, thumbnailPath);
}
