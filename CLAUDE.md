...
- **MobileHUD is NOT a UI root — it's a `TouchScreenControls` setup call.** It lives in `src/mobile-hud.ts` **outside `src/ui/`** on purpose: `refreshRoots` scans `src/ui/*.tsx` and would import any component-named file into the generated aggregator and break scene compile. So MobileHUD has its own selection flag (`mobileHudSelected`), its own RightPanel branch, and its own store (`components/UIDesigner/MobileHud/`). The module + `src/index.ts` wiring are written **lazily** (only once the config leaves SDK defaults, `isDefaultMobileHudConfig`) and **auto-cleaned** when reset to defaults — untouched scenes carry no no-op call. "Hide Input Actions" is derived (all buttons hidden), not a stored/emitted field. See [`docs/UIDesigner.md`](docs/UIDesigner.md) → "MobileHUD".
...
Skills live in `.ai/skills/*/SKILL.md`. Read the relevant `SKILL.md` when a task matches a skill's domain. Third-party skills are pinned in the committed `skills-lock.json`; on a fresh clone run `make install-skills`. It runs `npx skills@1.7.0 experimental_install` (pinned in the Makefile; bump it deliberately), which restores the lock only into `.agents/skills/` (a folder Claude Code does not read), then symlinks each skill into `.claude/skills/`. Both folders are gitignored. To add a skill, run `npx skills@1.7.0 add <source> --skill <name…> -a claude-code` (names space-separated; a comma-joined `a,b` matches nothing) and commit the updated lock. Install personal skills with `-g` so they stay out of the lock.

**Figma / design work:** on any Figma cue (a `figma.com` URL or the word "figma"), read [`.ai/skills/figma/SKILL.md`](.ai/skills/figma/SKILL.md) FIRST — the project umbrella for Figma→code work (leaf skills, the browser fallback when the Figma MCP is capped, palette→`vars.css`/`DESIGN.md` sync, live verification, SVG/CSS gotchas), which makes the "Design handoff" note above actionable. A `UserPromptSubmit` hook (`.ai/hooks/figma/hook.py`) fires the reminder automatically. The shell `case` check in front of it in `.claude/settings.json` is the only way to skip starting Python on non-Figma prompts: `UserPromptSubmit` ignores `matcher`, and an `if` field stops the hook from running at all.
...
- [`docs/testing-standards.md`](docs/testing-standards.md) — how to write a test here, plus every testing gotcha: Vitest conventions and mocking traps (`vi.mock` factories replacing whole modules, fake timers leaking across `describe`s), and E2E patterns (real keyboard input vs `fill()`, locators vs `ElementHandle`s, focus-actually-on-element gates, outcome waits vs fixed sleeps). Read before writing or debugging any test. It also names e2e scripts, helper files, exported functions and `data-testid` locators **verbatim**, so it goes stale silently — nothing lints prose against code. Changing a `test:e2e*` script, `e2e.yml`, or an `e2e/helpers/*` export means updating this doc in the SAME commit; three stale references reached review on one branch (`installLiveWallet`/`live-wallet.ts`, `liveAuthDappUrl`, and a deleted `deploying-jump` locator).
