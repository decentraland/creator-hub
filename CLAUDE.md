...
### GLB image URIs must not contain `..` (Bevy explorer)

The Bevy explorer maps a GLB's relative image URI onto the scene content by plain string: `crates/ipfs/src/ipfs_path.rs` `normalize_path` only swaps backslashes and never collapses `..`, so `../shared/tex.png` misses the content map and the material renders with no base color (only its emissive map shows). The Unity explorer normalizes and hides the problem. Anything that writes or rewrites texture references — the Creator Hub optimizer's sidecars (`main/src/modules/optimizer`), asset-pack tooling — must place shared textures beside or below the GLB, never in a parent folder. The optimizer pipeline test asserts no `..` in any written URI.
