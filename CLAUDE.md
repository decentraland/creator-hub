...
### GLB image URIs with `..` need bevy-explorer#1245

The optimizer's shared sidecar folder makes GLBs reference textures through `../` URIs. Bevy explorer builds before decentraland/bevy-explorer#1245 mapped such a URI onto the scene content by plain string (`ipfs_path.rs` `normalize_path` only swapped backslashes), so the lookup missed and models rendered with only their emissive map; Unity always normalized. If that symptom reappears, the client is stale — the fix belongs in the explorer, not in relocating textures beside each model (that was tried, and it forfeits cross-folder dedup).
